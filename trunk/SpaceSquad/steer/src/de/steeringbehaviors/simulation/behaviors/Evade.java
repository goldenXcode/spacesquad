...
* Evade
*
* Evades a specified target object. This is not the same as
* the seek behavior! In Seek the current position of the target 
* is the destination point. The evade behavior tries to 
* anticipate the movement of the target and uses the estimated
* future position of the target for the force calculation.
* 
*/
public class Evade extends Behavior
{
	 ...
	 */
	public Evade(Geometrie target, double influence) 
	{ 
		m_target = target; 
	...
	{ 
		...
		des_vel = veh.getPos().sub(m_estimatePos);
		distance = des_vel.length();
