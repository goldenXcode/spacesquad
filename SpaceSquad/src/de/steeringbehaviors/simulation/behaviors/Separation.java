...
*   class Separation
*
*   Implements the Separation behaviour
*/
public class Separation extends Behavior
{
	...
	*/
	public Separation(int nearAreaRadius, int influence)
	{  
		m_neighborhood = new Neighborhood();
		m_nearVehicles = new Vector();
	...
	*/
	...
	public Vector2d calculate(Vehicle v)
	{
		...
		{			
			...
			Vector2d temp;
			temp = neighbor_vehicle.getPos().sub(v.getPos());
			temp.scale(-1/temp.length());
				
			steering = steering.add(temp);										
		}
		
		steering.scale(m_influence);
