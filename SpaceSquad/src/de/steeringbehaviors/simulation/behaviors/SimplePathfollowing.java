...
public class SimplePathfollowing extends Behavior implements ObjectAttributes
{
	...
	*/
	...
	public int i_waypointCounter=0;
	int i_waitAtWaypoint;
	int i_waitAtWaypointTimer;
	//tnind
	public void waitAtWaypoint(int i_wayPoint , int i_numberofCalsToSkip)
	{
		i_waitAtWaypoint = i_wayPoint;
		i_waitAtWaypointTimer = i_numberofCalsToSkip;
	}
	...
	{ 
		...
		if(i_waypointCounter == i_waitAtWaypoint && i_waitAtWaypointTimer >0)
		{
			//stop the vehicle
			veh.setVel(new Vector2d(0,0));
			i_waitAtWaypointTimer--;
	...
	public int m_stop = STOP_AT_TARGET;
}